...
public abstract class SimulationModel<T extends DefaultSimulationConfig> extends DefaultModel {
	...
	public SimulationModel(final T config) {
		...
		this.colorMap.put(SIRType.ID_SUSCEPTIBLE.ordinal(), config.getPedestrianSusceptibleColor());
		this.colorMap.put(SIRType.ID_INFECTED.ordinal(), config.getPedestrianInfectedColor());
		this.colorMap.put(SIRType.ID_RECOVERED.ordinal(), config.getPedestrianRecoveredColor());
